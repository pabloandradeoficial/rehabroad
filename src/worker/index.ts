...
app.get("/api/student/progress", optionalAuthMiddleware, async (c) => {
  ...
  return c.json({
    ...
    progress: {
      ...progress,
      modules_visited: JSON.parse(progress.modules_visited || "[]")
    }
  }, 200);
});
...
app.post("/api/student/progress", optionalAuthMiddleware, async (c) => {
  ...
  if (results.length === 0) {
    ...
    let modulesVisited = JSON.parse(current.modules_visited || "[]");
...
app.get("/api/admin/students", authMiddleware, async (c) => {
  ...
  // Check admin
  const { results: subResults } = await c.env.DB.prepare(
    "SELECT is_admin FROM subscriptions WHERE user_id = ?"
  ).bind(user!.id).all();
  
  if (subResults.length === 0 || (subResults[0] as any).is_admin !== 1) {
  ...
  const students = results.map((s: any) => ({
    ...s,
    modules_visited: JSON.parse(s.modules_visited || "[]")
  }));
...
app.post("/api/beta-waitlist", async (c) => {
  ...
  if (existing) {
    return c.json({ message: "E-mail já cadastrado na lista de espera" }, 200);
  }
...
app.get("/api/dashboard/stats", authMiddleware, async (c) => {
  ...
  if (patientIdList.length > 0) {
    ...
    const evalResult = await c.env.DB.prepare(
      `SELECT COUNT(*) as count FROM evaluations WHERE patient_id IN (${patientIdList.join(",")})`
    ).first() as any;
    totalEvaluations = evalResult?.count || 0;

    // Get total evolutions
    const evolResult = await c.env.DB.prepare(
      `SELECT COUNT(*) as count FROM evolutions WHERE patient_id IN (${patientIdList.join(",")})`
    ).first() as any;
...
app.get("/api/dashboard/charts", authMiddleware, async (c) => {
  ...
  const { results: weeklyEvols } = await c.env.DB.prepare(
    `SELECT 
      ...
      COUNT(*) as count
    FROM evolutions 
    WHERE patient_id IN (${patientIdList.join(",")})
...
const adminMiddleware = async (c: any, next: any) => {
  ...
  if (!subscription || subscription.is_admin !== 1) {
